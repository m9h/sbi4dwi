...
def spec_k(K, dpar_range=(0.3, 2.6)):
    names, lo, hi = [], [], []
    for k in range(K):
...
def make_forward_k(K, bvals_si, bvecs):
    cfg = PrismConfig(n_fibres=K); bv = jnp.asarray(bvals_si); gv = jnp.asarray(bvecs)
    ...
    def one(p):
        ...
        dd = jnp.stack(dirs)
        f = p[2 * K:3 * K]; fi = p[3 * K]; dpar = p[3 * K + 1] * 1e-9
...
def dirs_fracs_from_samples(s, K):
...
def main():
    ...
    a = ap.parse_args()
    bvals, bvecs = load_scheme(a.scheme); names, lo, hi = spec_k(a.K)
    fwd = make_forward_k(a.K, bvals, bvecs)
    ...
    np.savez(a.out + ".meta.npz", names=np.array(names), lo=lo, hi=hi, K=a.K, scheme=a.scheme, hidden=a.hidden, depth=a.depth,
             losses=losses)
